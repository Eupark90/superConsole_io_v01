...
| PC2 | Caps Lock LED |
| PC3 | Num Lock LED |
| PC6 | LCD/OLED 밝기 PWM (TIM3 CH1, 기본 0%) |
...
- `0%`는 PWM duty 0으로 백라이트/디스플레이 off 용도입니다.
- 버튼을 누른 순간만 처리하므로 누르고 있는 동안 반복 증감하지 않습니다.
- 밝기 변경 시 OLED가 `LIGHT xx%` 형식의 전용 큰 글씨 화면으로 약 1.6초 전환됩니다.
...
| `SET BL 60` 또는 `BL 60` | 밝기 설정, 허용값 `0/20/40/60/80/100` |
...
- `HELP`
- `GET BL`, `BL`
- `SET BL <0|20|40|60|80|100>`, `BL <0|20|40|60|80|100>`
- `GET BAT`
